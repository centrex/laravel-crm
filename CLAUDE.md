...
`laravel-package-laravel-crm` — Template/scaffold for creating new Laravel packages in this monorepo.

Namespace: `Centrex\Crm\` (replace with actual vendor/package name)

This is the reference template. When creating a new package, copy this directory and do a find-and-replace on `Centrex`, `Crm`, `laravel-crm`, and `package-description`.
...
1. Replace all `Centrex\Crm` namespace references
2. Update `composer.json` name, description, and autoload namespace
3. Rename `Crm.php` and `CrmServiceProvider.php`
